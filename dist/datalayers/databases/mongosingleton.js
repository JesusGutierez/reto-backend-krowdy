...
class MongoSingleton {
    ...
    createConnection() {
        this.connection = mongoose_1.default.connect(process.env.DB_HOST);
    }
    validateConnection() {
        if (!this.connection)
...
const comidaSchema = new mongoose_1.Schema({
    type: { enum: ['vegetables', 'fruits'] }
});
const mongoConnectionHandler = new MongoSingleton();
const connectionDB = mongoConnectionHandler.getConnection();
